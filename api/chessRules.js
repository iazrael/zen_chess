import { PieceType, Color } from './common/types.js';
import { BOARD_ROWS, BOARD_COLS } from './common/constants.js';
// Helper to check bounds
const isValidPos = (x, y) => x >= 0 && x < BOARD_COLS && y >= 0 && y < BOARD_ROWS;
// Get piece at position
const getPiece = (board, pos) => {
...
const crossedRiver = (y, color) => {
    if (color === Color.Red)
        return y < 5;
    return y > 4;
};
export const getValidMovesForPiece = (board, pos) => {
    ...
    switch (piece.type) {
        ...
        case PieceType.Elephant: // Diagonal 2 steps, no river crossing, blockable eye
            [[2, 2], [2, -2], [-2, 2], [-2, -2]].forEach(([dx, dy]) => {
                ...
                if (color === Color.Red && ty < 5)
                    return;
                if (color === Color.Black && ty > 4)
                    return;
                // Check eye (midpoint)
                const eyeX = x + dx / 2;
                const eyeY = y + dy / 2;
...
export const getLegalMoves = (board, pos) => {
...
export const fenToMoveString = (from, to) => {
...
export const parseMoveString = (str) => {
...
export const evaluateBoard = (board, playerColor) => {
    let score = 0;
    const values = {
