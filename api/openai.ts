...
**核心原则**:
1. **拒绝送子**: 列表里标有 "[有根!小心!]" 的走法意味着你吃子后会被反杀，你需要评估是否值得（如以小博大）。
2. **大局观**: 开局抢出车，中局控肋道，残局兵归心。
3. **进攻性**: 除非有危险丢子危险，否则你必须以吃子和将军为目标。
4. **预判**: 你要考虑对方上一步棋落子后，影响哪些棋子或局势，优先保护价值高的棋子，如车、马、炮等，兵、卒等可以适当放弃。
5. **关于坐标**: 输入的 X 轴坐标是 0-8。
   - 你的记谱习惯是 1-9。
...
请选择一步最符合棋理的走法，并以 JSON 格式返回你的选择。`;
};
