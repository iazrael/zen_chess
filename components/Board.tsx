...
export const Board: React.FC<BoardProps> = ({ board, onSquareClick, selectedPos, validMoves, lastMove }) => {
  // 9 cols, 10 rows
  // We render a grid of cells, but pieces sit on intersections.
  // Actually, simpler to render the grid lines as SVG background, and then render 90 divs (9x10) on top.
  
  return (
    <div className="relative w-full max-w-[600px] aspect-[9/10] bg-wood-500 rounded-lg shadow-2xl p-4 select-none border-4 border-wood-700">
      
      {/* The Grid Lines (SVG) */}
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-0 p-[inherit]" viewBox="0 0 90 100" preserveAspectRatio="none">
        <defs>
        ...
        {/* Background Wood Texture hint */}
        <rect x="0" y="0" width="90" height="100" fill="transparent" />
        ...
        {/* Position Markers (Little corners) - simplified */}
        {[
            [1,2], [7,2], // Cannons Top
      ...
      <div className="relative z-10 grid grid-rows-10 grid-cols-9 w-full h-full">
        {board.map((row, y) => (
            row.map((piece, x) => {
                ...
                return (
                    ...
                    >
                        {/* Move Indicator Dot */}
                        {isValidMove && !piece && (
                            <div className="absolute w-3 h-3 bg-green-500/50 rounded-full animate-pulse pointer-events-none"></div>
                        )}
                        
                        {/* Capture Indicator Ring */}
                        {isValidMove && piece && (
                            <div className="absolute w-[90%] h-[90%] border-4 border-red-500/50 rounded-full animate-pulse pointer-events-none"></div>
                        )}
